...
exports.deleteProduct = async (req, res, next) => {
  try {
    ...
    if (!deleteItem.deletedCount) {
      return res
        .status(400)
        .json({ status: "Failed", error: "Couldn't delete the product" });
    }
    res.status(200).json({
      status: "Success",
...
exports.bulkDeleteProduct = async (req, res, next) => {
  try {
    const deleteItem = await bulkDeleteProductService(req.body.ids);
    res.status(200).json({
      status: "Success",
      message: "Data bulk deleted  successfully!",
      data: deleteItem,
    });
  } catch (error) {
    res.status(400).json({
      status: "Failed",
      message: "Data is not bulk deleted",
      error: error.message,
    });
  }
};
